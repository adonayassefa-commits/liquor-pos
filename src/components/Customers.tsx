  ...
  return (
    <div className="min-h-screen bg-[#1c1815] p-6">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-[#f2ece2]">Customers</h1>
        <button
      ...
      {loading ? (
        ...
        <p className="text-[#8a8177]">No customers yet.</p>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
          {filtered.map((c) => (
            <div key={c.id} className="bg-[#2c2419] p-4 rounded-lg border border-[#33291f]">
              <h3 className="text-[#f2ece2] font-bold">{c.name}</h3>
              {c.phone && <p className="text-[#8a8177] text-sm">{c.phone}</p>}
              {c.email && <p className="text-[#8a8177] text-sm">{c.email}</p>}
            </div>
          ))}
        </div>
      )}
  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center p-4 z-50">
      <form onSubmit={handleSubmit} className="bg-[#2c2419] p-6 rounded-lg w-full max-w-md space-y-3">
        <h2 className="text-xl font-bold text-[#f2ece2] mb-2">Add Customer</h2>
