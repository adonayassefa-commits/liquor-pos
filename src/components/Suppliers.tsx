  ...
  return (
    <div className="min-h-screen bg-[var(--bg-page)] p-6">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-xl font-semibold text-[var(--text-primary)]" style={{ fontFamily: 'Georgia, serif' }}>Suppliers</h1>
        <button
      ...
      {loading ? (
        ...
        <p className="text-[var(--text-muted)]">No suppliers yet.</p>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {suppliers.map((s) => (
            <div key={s.id} className="bg-[var(--bg-card)] p-4 rounded-xl border border-[var(--border)] shadow-sm">
              <div className="flex justify-between items-start">
                <h3 className="text-[var(--text-primary)] font-semibold">{s.name}</h3>
                <button
              ...
              {s.contact_person && <p className="text-[var(--text-muted)] text-sm">Contact: {s.contact_person}</p>}
              {s.phone && <p className="text-[var(--text-muted)] text-sm">Phone: {s.phone}</p>}
              {s.email && <p className="text-[var(--text-muted)] text-sm">Email: {s.email}</p>}
              {s.payment_terms && <p className="text-[var(--text-muted)] text-sm">Terms: {s.payment_terms}</p>}
            </div>
          ))}
        </div>
      )}
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center p-4 z-50">
      <form onSubmit={handleSubmit} className="bg-[var(--bg-card)] p-6 rounded-xl w-full max-w-md space-y-3 shadow-lg">
        ...
        <div>
          <label className="block text-sm text-[var(--text-secondary)] mb-1">Payment Terms</label>
